...
__global__ void matrixSub(float *matrixA,float *matrixB,float *matrixC)
{
	int idx = (gridDim.y * blockIdx.x + blockIdx.y )*blockDim.x*blockDim.y*blockDim.z + blockDim.z*blockDim.y*threadIdx.x + blockDim.z*threadIdx.y + threadIdx.z;	
...
int main(int argc,char **argv)
{
	...
	int blockDimX = BX;
	int blockDimY = BY;
	float * matrixA,* matrixB, *matrixC,* matrixD,*matrixC_d, *matrixD_d;
	...
	matrixD = (float *) malloc(nBytes);
	matrixC_d = (float *) malloc(nBytes);

	float * d_matrixA,* d_matrixB, * d_matrixC ,* d_matrixD;
	...
	matrixSub<<<b,g>>>(d_matrixA,d_matrixB,d_matrixD);
	...
	check_result(matrixD,matrixD_d,nx,ny);
	cudaFree(d_matrixA);
	cudaFree(d_matrixB);
	cudaFree(d_matrixC);
	cudaFree(d_matrixD);
